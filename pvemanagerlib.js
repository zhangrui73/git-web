...
Ext.define('PVE.Utils', { utilities: {
   ...
   // noSubKeyHtml: 'You do not have a valid subscription for this server. Please visit <a target="_blank" href="http://www.proxmox.com/products/proxmox-ve/subscription-service-plans">www.proxmox.com</a> to get a list of available options.',
	...
	var iconCls = 'question-circle'; //菜单项显示的图标
	switch (value) {
	    case 'AVAIL': //avail 有用的
	    case 'ONLINE':
		iconCls = 'check-circle good';
		break;
	    case 'REMOVED':
	    case 'DEGRADED': //degraded 退化的
		iconCls = 'exclamation-circle warning';
		break;
	    case 'UNAVAIL': //unavail 
	    case 'FAULTED': //faulted 有毛病的
	    case 'OFFLINE': //offline 不在线
	...
	vmware: gettext('VMware compatible'), //compatible 兼容性
	qxl: 'SPICE',
...
Ext.define('PVE.node.Summary', {
	...
	var win = Ext.create('Ext.window.Window', {
	    ...
	    width: 600,
	    height: 400,
	    layout: 'fit',//其实就是EXT中的布局，常用的有border、column、fit、form、tabel这几种。
	    modal: true,
	    items: [ view ]
	});
	...
	var version_btn = new Ext.Button({
	    ...
	    handler: function(){
		Proxmox.Utils.checked_command(function() { me.showVersions(); });  //隐藏安装包的版本号
		}
	});
...
Ext.define('PVE.dc.BackupView', {
	...
	fields: [ 
	    ...
	    'mailto',
	    { name: 'enabled', type: 'boolean' }, //使能够
	    { name: 'all', type: 'boolean' },
	    { name: 'snapshot', type: 'boolean' }, //快照
	    { name: 'stop', type: 'boolean' },
	    { name: 'suspend', type: 'boolean' }, //延缓
	    { name: 'compress', type: 'boolean' } //压缩
	]
    });
});
...
Ext.define('PVE.StdWorkspace', {
	...
	if (Proxmox.UserName) {
	    var msg =  Ext.String.format(gettext(" "), "'" + Proxmox.UserName + "'");
	...
	Ext.apply(me, {
	    ...
	    items: [
		{
		    ...
		    items: [
			{
				...
				'<img style="padding-top:4px;padding-right:25px" src="/pve2/images/proxmox_logo.png"/></a>'
			},
			//{
			   // minWidth: 150,
			   // id: 'versioninfo',
